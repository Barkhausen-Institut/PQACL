...
// Function to get the B_i from the server
void ConnectionHandler::getB_i_from_server(const std::string& bi_export_path){
    Client client = Client(this->ServerIP, this->ServerPort);
    std::string msg = "getB_i";
    client.send(msg);
    client.receive_file(bi_export_path);
}
...
// Function to send the data and proof to the server
void ConnectionHandler::sendDataToServer(const std::string& data_path){
    Client client = Client(this->ServerIP, this->ServerPort);
    std::string msg = "validateAccessPre";
    client.send(msg);
    client.send_file(data_path);
}